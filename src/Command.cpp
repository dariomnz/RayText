...
void Command::Consume(App *app)
{
    ...
    if (command[0] == '>')
    {
        ...
        {
            DEBUG(">save");
            TextFile_Save(&app->editor.currentTextFile);
        }
    ...
    else if (command[0] == ':')
    {
        ...
        if (*endptr == '\0')
        {
            ...
            if (val - 1 < (long)app->editor.currentTextFile.buffer.size() && val > 0)
            {
                app->editor.currentTextFile.cursor.line_num = val - 1;
                app->editor.currentTextFile.cursor.position = 0;
            }
        }
    }
    command.clear();
